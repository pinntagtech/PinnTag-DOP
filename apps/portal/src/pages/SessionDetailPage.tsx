...
import {
  ArrowLeft, Play, Wand2, Sparkles, Star,
  CheckCircle2, Send, ChevronRight,
  AlertCircle, AlertTriangle, Upload, RefreshCw,
  MoreHorizontal, Images, ArrowRightLeft,
  CheckCircle, Zap,
  Image as ImageIcon,
} from 'lucide-react';
import {
...
import {
  BotProgressCompact,
} from '../components/ui/BotProgressBar';
...
const PIPELINE_ACTIONS: Record<string, {
  action: string;
  label: string;
  ...
  draft: [
    { action: 'validate', label: 'Run validation',
      icon: <Play size={14} />, variant: 'primary' },
  ],
  validated: [
    { action: 'transform', label: 'Run transformation',
      icon: <Wand2 size={14} />, variant: 'primary' },
  ],
  transformed: [
    { action: 'enrich', label: 'Run enrichment',
      icon: <Sparkles size={14} />, variant: 'primary' },
  ],
  enriched: [
    { action: 'approve', label: 'Approve for publishing',
      icon: <CheckCircle2 size={14} />, variant: 'primary' },
  ],
  ready: [
    { action: 'publish', label: 'Publish',
      icon: <Send size={14} />, variant: 'primary' },
  ],
};
...
export default function SessionDetailPage() {
  ...
  const [selectedRecord, setSelectedRecord] =
    useState<SeedingRecord | null>(null);
  ...
  const handleResetBotStage = async (
    stage: 'gallery' | 'menu' | 'reviews',
  ) => {
    if (!id) return;
    const label =
    ...
    try {
      ...
      const nice =
        ...
        : 'Reviews';
      setBotToast(
        `${nice} data reset. You can now re-fetch using the ` +
          `button above.`,
      );
    ...
    <div style={{ display: 'flex', flexDirection: 'column',
          ...
          <div>
            ...
            <p style={{
              ...
              <span style={{
                ...
                padding: '2px 6px',
                borderRadius: '4px',
                backgroundColor: session.environment === 'production'
                  ? '#0A0A0A'
                  ...
                  ? '#04785720'
                  : '#2563EB20',
                color: session.environment === 'production'
                  ? '#ffffff'
                  : session.environment === 'staging'
                  ? '#7C3AED'
                  : session.environment === 'pre-prod'
              ...
              {showAdminMenu && (
                <>
                  ...
                  <div style={{
                    ...
                    {(['gallery', 'menu', 'reviews'] as const).map(
                      (stage) => {
                        const label =
                          stage === 'gallery'
                            ? 'Reset gallery data'
            ...
            <div style={{
              ...
              {rows.map((r) => (
                <Fragment key={r.key}>
                  ...
                  <span style={{ color: r.pending > 0 ? '#A16207' : '#94A3B8' }}>
                    Pending: <strong>{r.pending}</strong>
                  </span>
                  <span style={{ color: r.running > 0 ? '#1D4ED8' : '#94A3B8' }}>
            ...
            return (
              ...
              >
                ...
                <span style={{
                  ...
                  color:
                    j.status === 'running' ? '#1D4ED8' : '#A16207',
                  textTransform: 'uppercase',
                  letterSpacing: '0.05em',
                  fontWeight: 600,
                }}>
      ...
      {session.type === 'migration' && session.migratedFrom && (
        <div style={{
          ...
          <span>
            Migrated from{' '}
            <strong>{session.migratedFrom.sessionName}</strong>
            {' '}({session.migratedFrom.environment}){' '}
            on{' '}
            {new Date(session.migratedFrom.migratedAt)
              .toLocaleDateString()}
          </span>
        </div>
      )}
      ...
      {(() => {
        ...
        const entries: {
          ...
          { key: 'galleryMenu', label: 'Gallery/Menu', stat: ops.galleryMenu as any },
          ...
          <div style={{
            ...
            {entries.map((e) => (
              ...
              >
                ...
                <span>
                  {formatDistanceToNow(
                    new Date(e.stat.lastRunAt as string),
                    { addSuffix: true },
                  )}
                </span>
              </div>
      ...
      <div style={{
        ...
        {(['records', 'logs'] as Tab[]).map((tab) => {
          ...
          const count =
            ...
            tab === 'logs' ? logs?.length : undefined;
          return (
            <button
              key={tab}
        ...
        <div style={{
          ...
          <div style={{
            ...
            {records && records.length > 0 && (
              selectedRecordIds.length > 0 || session.status === 'enriched'
            ) && (
              <div style={{
                display: 'flex',
                alignItems: 'center',
                ...
                <div style={{
                  ...
                  {selectedRecordIds.length > 0 && (
                    <Button
                      ...
                      onClick={() =>
                        handleReEnrich(selectedRecordIds)
                      }
                    >
                      Re-enrich selected
                    </Button>
              ...
              <table style={{
                ...
                <thead>
                  <tr style={{
                    ...
                    {['Module', 'Status', 'Validation',
                      'Published ID', 'Bot data', 'Updated'].map((h) => (
                      <th key={h} style={{
                        textAlign: 'left',
                        fontSize: '11px',
                        textTransform: 'uppercase',
                ...
                <tbody>
                  {records.map((r: SeedingRecord) => {
                    ...
                    const isChecked =
                      selectedRecordIds.includes(r._id);
                    const errorCount = r.validationErrors
                      ?.filter(e => e.severity === 'error')
                      .length ?? 0;
                    const warnCount = r.validationErrors
        ...
        >
          ...
          >
            ...
            >
              ...
              {botEligibleRecords.length} published businesses and save them
              to the target DB.
            </p>
            <div
              style={{
      ...
      {showGalleryModal && (
        <div
          ...
          onClick={() =>
            !triggerBot.isPending && setShowGalleryModal(false)
          }
        >
          <div
            onClick={(e) => e.stopPropagation()}
          ...
          >
            ...
            >
              This will scrape Google Maps gallery photos (folder by folder)
              and menu highlights for {botEligibleRecords.length} published
              businesses.
            </p>
            <div
              style={{
